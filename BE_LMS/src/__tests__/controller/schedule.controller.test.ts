...
describe("📅 Schedule Controller Unit Tests", () => {
  ...
  describe("getTimeSlotsHandler", () => {
    ...
    it("should filter time slots by isActive=true", async () => {
      const mockSlots = [timeSlot];
      ...
      expect(scheduleService.getAllTimeSlots).toHaveBeenCalledWith({ isActive: true });
      expect(mockRes.success).toHaveBeenCalledWith(200, {
        message: "Time slots retrieved successfully",
        data: mockSlots,
      });
    ...
    it("should filter time slots by isActive=false", async () => {
      ...
      expect(scheduleService.getAllTimeSlots).toHaveBeenCalledWith({ isActive: false });
      expect(mockRes.success).toHaveBeenCalledWith(200, {
        message: "Time slots retrieved successfully",
        data: mockSlots,
      });
    ...
    it("should handle service errors", async () => {
      ...
      (scheduleSchemas.timeSlotFilterSchema.parse as jest.Mock).mockReturnValue({});
      (scheduleService.getAllTimeSlots as jest.Mock).mockRejectedValue(error);
  ...
  describe("createScheduleRequestHandler", () => {
    ...
    it("should create a schedule request with multiple slots", async () => {
      const scheduleData = {
        courseId,
        slots: [
          {
      ...
      const mockCreatedSchedules = [schedule, { ...schedule, dayOfWeek: DayOfWeek.WEDNESDAY }, { ...schedule, dayOfWeek: DayOfWeek.FRIDAY }];
      ...
      expect(mockRes.success).toHaveBeenCalledWith(201, {
        message: "Schedule request created successfully with 3 slots",
        data: mockCreatedSchedules,
      });
    });

    it("should use current user's ID as teacherId", async () => {
      const scheduleData = {
        courseId,
    ...
    it("should handle service errors", async () => {
      ...
      mockReq.body = scheduleData;
      (scheduleSchemas.createScheduleSchema.parse as jest.Mock).mockReturnValue(scheduleData);
      (scheduleService.createScheduleRequest as jest.Mock).mockRejectedValue(error);
  ...
  describe("getTeacherScheduleHandler", () => {
    ...
    it("should get teacher schedule without date (defaults to current week)", async () => {
      const mockSchedule = {
        teacher: { _id: teacherId, fullname: "John Doe" },
        schedule: [schedule],
        weekRange: { start: new Date(), end: new Date() },
      ...
      mockReq.query = {};
      (scheduleSchemas.teacherIdSchema.parse as jest.Mock).mockReturnValue(teacherId.toString());
      (scheduleSchemas.teacherScheduleQuerySchema.parse as jest.Mock).mockReturnValue({});
      (scheduleService.getTeacherWeeklySchedule as jest.Mock).mockResolvedValue(mockSchedule);

      await getTeacherScheduleHandler(mockReq as Request, mockRes as Response, mockNext);

      expect(scheduleService.getTeacherWeeklySchedule).toHaveBeenCalledWith(teacherId.toString(), undefined, undefined);
      expect(mockRes.success).toHaveBeenCalledWith(200, {
        message: "Teacher schedule retrieved successfully",
        data: mockSchedule,
      });
    ...
    it("should handle service errors", async () => {
      ...
      (scheduleSchemas.teacherIdSchema.parse as jest.Mock).mockReturnValue(teacherId.toString());
      (scheduleSchemas.teacherScheduleQuerySchema.parse as jest.Mock).mockReturnValue({});
      (scheduleService.getTeacherWeeklySchedule as jest.Mock).mockRejectedValue(error);
  ...
  describe("getCourseScheduleHandler", () => {
    ...
    it("should get course schedule without status filter", async () => {
      const mockSchedule = {
        course: { _id: courseId, title: "Mathematics 101" },
        schedule: [schedule],
      };
      ...
      mockReq.query = {};
      (scheduleSchemas.courseIdSchema.parse as jest.Mock).mockReturnValue(courseId);
      (scheduleSchemas.courseScheduleQuerySchema.parse as jest.Mock).mockReturnValue({});
      (scheduleService.getCourseSchedule as jest.Mock).mockResolvedValue(mockSchedule);

      await getCourseScheduleHandler(mockReq as Request, mockRes as Response, mockNext);

      expect(scheduleService.getCourseSchedule).toHaveBeenCalledWith(courseId, undefined);
      expect(mockRes.success).toHaveBeenCalledWith(200, {
        message: "Course schedule retrieved successfully",
        data: mockSchedule,
      });
    ...
    it("should handle service errors", async () => {
      ...
      (scheduleSchemas.courseIdSchema.parse as jest.Mock).mockReturnValue(courseId);
      (scheduleSchemas.courseScheduleQuerySchema.parse as jest.Mock).mockReturnValue({});
      (scheduleService.getCourseSchedule as jest.Mock).mockRejectedValue(error);
  ...
  describe("approveScheduleRequestHandler", () => {
    ...
    it("should reject schedule request successfully", async () => {
      ...
      (scheduleSchemas.scheduleIdSchema.parse as jest.Mock).mockReturnValue(scheduleId);
      (scheduleSchemas.approveScheduleSchema.parse as jest.Mock).mockReturnValue(approvalData);
      (scheduleService.approveScheduleRequest as jest.Mock).mockResolvedValue(rejectedSchedule);
    ...
    it("should handle service errors", async () => {
      ...
      (scheduleSchemas.scheduleIdSchema.parse as jest.Mock).mockReturnValue(scheduleId);
      (scheduleSchemas.approveScheduleSchema.parse as jest.Mock).mockReturnValue(approvalData);
      (scheduleService.approveScheduleRequest as jest.Mock).mockRejectedValue(error);
  ...
  describe("getPendingRequestsHandler", () => {
    ...
    it("should return empty array when no pending requests", async () => {
      (scheduleService.getPendingScheduleRequests as jest.Mock).mockResolvedValue([]);
    ...
    it("should handle service errors", async () => {
      ...
      (scheduleService.getPendingScheduleRequests as jest.Mock).mockRejectedValue(error);
