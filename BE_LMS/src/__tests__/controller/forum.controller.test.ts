...
describe("📚 Forum Controller Unit Tests", () => {
    ...
    describe("listForumsHandler", () => {
        ...
        it("should filter forums by isActive status", async () => {
            ...
            expect(forumService.listForumsOfACourse).toHaveBeenCalled();
        });
        ...
        it("should handle service errors", async () => {
            ...
            mockReq.query = {courseId};
            (forumSchemas.listForumsSchema.parse as jest.Mock).mockReturnValue({courseId});
            (forumService.listForumsOfACourse as jest.Mock).mockRejectedValue(serviceError);
    ...
    describe("getForumByIdHandler", () => {
        ...
        it("should handle forum not found", async () => {
            const notFoundError = new Error("Forum not found");
            mockReq.params = {id: forumId};
            (forumSchemas.forumIdSchema.parse as jest.Mock).mockReturnValue(forumId);
            (forumService.getForumById as jest.Mock).mockRejectedValue(notFoundError);
    ...
    describe("createForumHandler", () => {
        ...
        it("should create forum with single file", async () => {
            const forumData = {courseId, title: "New Forum", description: "Description"};
            ...
            expect(forumService.createForum).toHaveBeenCalledWith(
                expect.objectContaining({courseId, createdBy: userId}),
                mockFile
            );
        });
        ...
        it("should handle service errors", async () => {
            ...
            mockReq.body = forumData;
            (forumSchemas.createForumSchema.parse as jest.Mock).mockReturnValue(forumData);
            (forumService.createForum as jest.Mock).mockRejectedValue(serviceError);
    ...
    describe("updateForumByIdHandler", () => {
        it("should update forum successfully", async () => {
            ...
            expect(forumService.updateForumById).toHaveBeenCalledWith(forumId, updateData, userId.toString(), Role.STUDENT, undefined);
            expect(mockRes.success).toHaveBeenCalledWith(200, {
        ...
        it("should update forum with files", async () => {
            ...
            expect(forumService.updateForumById).toHaveBeenCalledWith(forumId, updateData, userId.toString(), Role.STUDENT, mockFiles[0]);
        });
