...
ScheduleExceptionSchema.index({ scheduleId: 1, exceptionDate: 1 }, { unique: true });

// Find exceptions for a specific course
ScheduleExceptionSchema.index({ courseId: 1, exceptionDate: 1 });
...
ScheduleExceptionSchema.pre('save', function (next) {
  ...
  if (this.exceptionType === ExceptionType.RESCHEDULE && !this.newTimeSlotId) {
    return next(new Error('Rescheduled exception must have a new time slot'));
  }

  // If replacement, must have a replacement teacher
  if (this.exceptionType === ExceptionType.REPLACEMENT && !this.replacementTeacherId) {
    return next(new Error('Replacement exception must have a replacement teacher'));
  }
