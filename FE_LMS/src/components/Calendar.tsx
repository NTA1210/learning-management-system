...
const getTimeSlotInfo = (timeSlotId: TimeSlot | string): { startTime: string; endTime: string; slotName: string } => {
...
const isScheduleEffectiveOnDate = (schedule: Schedule, date: Date): boolean => {
  ...
  const effectiveTo = schedule.effectiveTo ? parseISO(schedule.effectiveTo) : addMonths(effectiveFrom, 12);
...
const Calendar: React.FC = () => {
  ...
  const [scheduleViewMode, setScheduleViewMode] = useState<ScheduleViewMode>('my-schedule');
  ...
  const [weeklySchedule, setWeeklySchedule] = useState<TeacherWeeklySchedule | null>(null);
  ...
  const [teacherSchedule, setTeacherSchedule] = useState<TeacherWeeklySchedule | null>(null);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const { courses: courseList } = await courseService.getAllCourses(params);
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      try {
        ...
        const schedule = await scheduleService.getTeacherSchedule(user._id, dateStr, 'approved');
  ...
  useEffect(() => {
    const fetchCourseSchedule = async () => {
      ...
      try {
        const schedules = await scheduleService.getCourseSchedule(selectedCourseId, 'approved');
        setCourseSchedules(schedules);
      } catch (err: unknown) {
        console.error('Failed to fetch course schedule:', err);
        const message = err instanceof Error ? err.message : 'Failed to load course schedule';
        setError(message);
        setCourseSchedules([]);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    const fetchSelectedTeacherSchedule = async () => {
      if (!selectedTeacherId || scheduleViewMode !== 'by-teacher' || !isAdmin) return;
  ...
  const handleApproveSchedule = async (scheduleId: string, approved: boolean, note?: string) => {
    setActionLoading(true);
    setActionError(null);

    try {
      await scheduleService.approveSchedule(scheduleId, { approved, approvalNote: note });
      ...
      if (user?._id) {
        const dateStr = format(currentDate, 'yyyy-MM-dd');
        const schedule = await scheduleService.getTeacherSchedule(user._id, dateStr);
        setWeeklySchedule(schedule);
      }
  ...
  const getSchedulesForDate = useCallback((day: Date): Schedule[] => {
  ...
  // Calculate stats
  const stats = useMemo(() => {
    const approvedCount = allSchedulesForWeek.filter(s => s.status === 'approved' || s.status === 'active').length;
    const pendingCount = allSchedulesForWeek.filter(s => s.status === 'pending').length;
    const uniqueCourses = new Set(allSchedulesForWeek.map(s =>
  ...
  const getCourseTitle = useCallback((courseId: Schedule['courseId']): string => {
    ...
    const courseIdStr = typeof courseId === 'object' ? (courseId as { _id: string })._id : courseId;
  ...
  const renderScheduleItem = (schedule: Schedule, index: number) => {
    ...
    return (
      <div
        ...
        style={{ borderLeftColor: color, borderLeft: `3px solid ${color}`, cursor: isAdmin ? 'pointer' : 'default' }}
        title={`${courseTitle} - ${timeSlot.startTime}-${timeSlot.endTime} @ ${schedule.location || 'TBA'}`}
            ...
            return (
              ...
              >
                ...
                <div className="day-schedules">
                  {daySchedules.slice(0, 3).map((schedule, i) => renderScheduleItem(schedule, i))}
                  {totalCount > 3 && (
                ...
                <div className="week-day-row">
                  ...
                  <div className="week-day-items">
                    {daySchedules.length === 0 ? (
                      <div className="no-class-indicator" style={{ color: '#9ca3af', fontSize: '12px', padding: '8px 0' }}>
                        No classes
                      </div>
                    ) : (
                      daySchedules.map((schedule, index) => {
                        ...
                        return (
                          <div
                            key={schedule._id}
                            className={`week-schedule-item-compact ${isAdmin ? 'clickable' : ''}`}
                            style={{ borderLeft: `3px solid ${color}`, cursor: isAdmin ? 'pointer' : 'default' }}
                            onClick={() => isAdmin && openScheduleDetail(schedule)}
                          >
                            ...
                            {schedule.location && (
                              <div className="room-info">📍 {schedule.location}</div>
                            )}
                          </div>
                        );
                      })
          ...
          {daySchedules.length === 0 ? (
            ...
            daySchedules.map((schedule, index) => {
              ...
              return (
                <div
                  key={schedule._id}
                  className={`day-schedule-item class ${isAdmin ? 'clickable' : ''}`}
                  style={{ borderLeftColor: color, cursor: isAdmin ? 'pointer' : 'default' }}
                  onClick={() => isAdmin && openScheduleDetail(schedule)}
                >
                  ...
                  <div className="schedule-content">
                    ...
                    <div className="schedule-teacher" style={{ fontSize: '12px', color: '#9ca3af', marginTop: '4px' }}>
                      👤 {teacherName}
                    </div>
  ...
  return (
    <>
        ...
        <div className="calendar-page-header">
          <div className="header-content">
            ...
            <div className="header-stats">
              ...
              <div className="stat-card">
                <div className="stat-number">{isAdmin ? pendingSchedules.length : stats.pendingRequests}</div>
        ...
        <div className="calendar-page-content">
          <section className="calendar-section">
            <div className="calendar-container">
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    ...
                    style={{
                      background: scheduleViewMode === 'my-schedule' ? '#3b82f6' : (darkMode ? '#374151' : '#f1f5f9'),
                      color: scheduleViewMode === 'my-schedule' ? 'white' : (darkMode ? '#e5e7eb' : '#374151'),
                      border: 'none',
                      borderRadius: '8px',
                      padding: '8px 16px',
                      cursor: 'pointer',
                  ...
                  <button
                    ...
                    style={{
                      background: scheduleViewMode === 'by-course' ? '#3b82f6' : (darkMode ? '#374151' : '#f1f5f9'),
                      color: scheduleViewMode === 'by-course' ? 'white' : (darkMode ? '#e5e7eb' : '#374151'),
                      border: 'none',
                      borderRadius: '8px',
                      padding: '8px 16px',
                      cursor: 'pointer',
                  ...
                  {isAdmin && (
                    <button
                      ...
                      style={{
                        background: scheduleViewMode === 'by-teacher' ? '#3b82f6' : (darkMode ? '#374151' : '#f1f5f9'),
                        color: scheduleViewMode === 'by-teacher' ? 'white' : (darkMode ? '#e5e7eb' : '#374151'),
                        border: 'none',
                        borderRadius: '8px',
                        padding: '8px 16px',
                        cursor: 'pointer',
                ...
                {scheduleViewMode === 'by-course' && (
                  <div ref={courseDropdownRef} style={{ position: 'relative', minWidth: '320px' }}>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        padding: '6px 14px',
                        borderRadius: '12px',
                        border: `2px solid ${showCourseDropdown ? '#3b82f6' : (darkMode ? '#4b5563' : '#e2e8f0')}`,
                        background: darkMode ? '#1f2937' : 'white',
                        cursor: 'pointer',
                        transition: 'all 0.2s',
                        boxShadow: showCourseDropdown ? '0 4px 12px rgba(59, 130, 246, 0.15)' : 'none',
                        height: '42px',
                    ...
                    >
                      {selectedCourseId ? (
                        <>
                          ...
                          {(() => {
                            const course = courses.find(c => c._id === selectedCourseId);
                            const colorIndex = courses.findIndex(c => c._id === selectedCourseId);
                            return course?.logo ? (
                              <img 
                                src={course.logo} 
                                alt={course.title}
                              ...
                              <div style={{
                                ...
                                height: '28px',
                                borderRadius: '6px',
                                background: `linear-gradient(135deg, ${COLORS[colorIndex % COLORS.length] || '#3b82f6'}, ${COLORS[(colorIndex + 2) % COLORS.length] || '#8b5cf6'})`,
                                display: 'flex',
                                alignItems: 'center',
                                ...
                                {course?.code?.substring(0, 3) || course?.title.substring(0, 2).toUpperCase() || '📚'}
                              </div>
                            );
                          })()}
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{
                              ...
                              {courses.find(c => c._id === selectedCourseId)?.title || 'Unknown Course'}
                            </div>
                            <div style={{
                              fontSize: '11px',
                              color: darkMode ? '#94a3b8' : '#64748b',
                            }}>
                              {courses.find(c => c._id === selectedCourseId)?.code || 'Course'}
                            </div>
                          </div>
                        </>
                      ) : (
                      ...
                      <i className={`bi bi-chevron-${showCourseDropdown ? 'up' : 'down'}`} style={{
                        color: darkMode ? '#9ca3af' : '#64748b',
                        marginLeft: 'auto',
                      }} />
                    </div>
                    ...
                    {showCourseDropdown && (
                      <div style={{
                        ...
                        {/* Search Input */}
                        <div style={{ padding: '12px', borderBottom: `1px solid ${darkMode ? '#374151' : '#e2e8f0'}` }}>
                          <div style={{
                            display: 'flex',
                            alignItems: 'center',
                            gap: '8px',
                            ...
                            <i className="bi bi-search" style={{ color: darkMode ? '#9ca3af' : '#64748b' }} />
                            <input
                              type="text"
                            ...
                            {courseSearchQuery && (
                              <i 
                                ...
                                style={{ color: darkMode ? '#6b7280' : '#9ca3af', cursor: 'pointer' }}
                                onClick={(e) => { e.stopPropagation(); setCourseSearchQuery(''); }}
                              />
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ maxHeight: '280px', overflowY: 'auto' }}>
                          {filteredCourses.length === 0 ? (
                            <div style={{
                              ...
                              <i className="bi bi-folder2-open" style={{ fontSize: '24px', marginBottom: '8px', display: 'block' }} />
                              No courses found
                            </div>
                          ) : (
                            filteredCourses.map((course, index) => (
                              <div
                                ...
                                style={{
                                  ...
                                  padding: '12px 14px',
                                  cursor: 'pointer',
                                  background: selectedCourseId === course._id 
                                    ? (darkMode ? 'rgba(59, 130, 246, 0.2)' : 'rgba(59, 130, 246, 0.1)')
                                    : 'transparent',
                                  borderLeft: selectedCourseId === course._id ? '3px solid #3b82f6' : '3px solid transparent',
                                  transition: 'all 0.15s',
                                }}
                                onMouseEnter={(e) => {
                                  if (selectedCourseId !== course._id) {
                                    e.currentTarget.style.background = darkMode ? '#374151' : '#f8fafc';
                                  }
                                }}
                                onMouseLeave={(e) => {
                                  if (selectedCourseId !== course._id) {
                              ...
                              >
                                ...
                                {course.logo ? (
                                  <img 
                                    ...
                                    style={{
                                      ...
                                      borderRadius: '8px',
                                      objectFit: 'cover',
                                      border: selectedCourseId === course._id 
                                        ? '2px solid #3b82f6' 
                                ...
                                {selectedCourseId === course._id && (
                                  <i className="bi bi-check-circle-fill" style={{ color: '#3b82f6', fontSize: '16px' }} />
                                )}
                              </div>
                            ))
                          )}
                ...
                {scheduleViewMode === 'by-teacher' && isAdmin && (
                  <div ref={teacherDropdownRef} style={{ position: 'relative', minWidth: '320px' }}>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        padding: '6px 14px',
                        borderRadius: '12px',
                        border: `2px solid ${showTeacherDropdown ? '#10b981' : (darkMode ? '#4b5563' : '#e2e8f0')}`,
                        background: darkMode ? '#1f2937' : 'white',
                        cursor: 'pointer',
                        transition: 'all 0.2s',
                        boxShadow: showTeacherDropdown ? '0 4px 12px rgba(16, 185, 129, 0.15)' : 'none',
                        height: '42px',
                        boxSizing: 'border-box',
                      }}
                    >
                          ...
                          {(() => {
                            ...
                            return teacher?.avatar_url ? (
                              ...
                              <div style={{
                                ...
                                {(teacher?.fullname || teacher?.username || 'T').substring(0, 2).toUpperCase()}
                              </div>
                            );
                          })()}
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{
                              ...
                              {teachers.find(t => t._id === selectedTeacherId)?.fullname || 
                               teachers.find(t => t._id === selectedTeacherId)?.username || 'Unknown'}
                            </div>
                            <div style={{
                              fontSize: '11px',
                              color: darkMode ? '#94a3b8' : '#64748b',
                              ...
                              <i className="bi bi-envelope" style={{ fontSize: '9px' }} />
                              {teachers.find(t => t._id === selectedTeacherId)?.email || ''}
                            </div>
                          </div>
                        </>
                      ) : (
                      ...
                      <i className={`bi bi-chevron-${showTeacherDropdown ? 'up' : 'down'}`} style={{
                        color: darkMode ? '#9ca3af' : '#64748b',
                        marginLeft: 'auto',
                      }} />
                    </div>
                          ...
                          <div style={{
                            ...
                            {teacherSearchQuery && (
                              <i 
                                className="bi bi-x-circle-fill" 
                                style={{ color: darkMode ? '#6b7280' : '#9ca3af', cursor: 'pointer' }}
                                onClick={(e) => { e.stopPropagation(); setTeacherSearchQuery(''); }}
                              />
                            )}
                          </div>
                        </div>
                        ...
                        <div style={{ maxHeight: '280px', overflowY: 'auto' }}>
                          {filteredTeachers.length === 0 ? (
                            <div style={{
                              ...
                              <i className="bi bi-person-x" style={{ fontSize: '24px', marginBottom: '8px', display: 'block' }} />
                              No teachers found
                            </div>
                          ) : (
                            filteredTeachers.map((teacher) => (
                              <div
                                ...
                                style={{
                                  ...
                                  padding: '12px 14px',
                                  cursor: 'pointer',
                                  background: selectedTeacherId === teacher._id 
                                    ? (darkMode ? 'rgba(16, 185, 129, 0.2)' : 'rgba(16, 185, 129, 0.1)')
                                    : 'transparent',
                                  borderLeft: selectedTeacherId === teacher._id ? '3px solid #10b981' : '3px solid transparent',
                                  transition: 'all 0.15s',
                                }}
                                onMouseEnter={(e) => {
                              ...
                              >
                                ...
                                {teacher.avatar_url ? (
                                  <img 
                                    ...
                                    style={{
                                      ...
                                      borderRadius: '50%',
                                      objectFit: 'cover',
                                      border: selectedTeacherId === teacher._id 
                                        ? '2px solid #10b981' 
                                        : `2px solid ${darkMode ? '#374151' : '#e2e8f0'}`,
                                    }}
                                  />
                                  ...
                                  <div style={{
                                    ...
                                    alignItems: 'center',
                                    justifyContent: 'center',
                                    color: selectedTeacherId === teacher._id ? 'white' : (darkMode ? '#9ca3af' : '#64748b'),
                                    fontWeight: 600,
                                    fontSize: '14px',
                                    flexShrink: 0,
                                  }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <div style={{
                                    ...
                                    {teacher.isVerified && (
                                      <i className="bi bi-patch-check-fill" style={{ color: '#3b82f6', fontSize: '12px' }} />
                                    )}
                                  </div>
                                  <div style={{
                                    fontSize: '11px',
                                    ...
                                    <i className="bi bi-envelope" style={{ fontSize: '10px' }} />
                                    {teacher.email}
                                  </div>
                                  {/* Specializations */}
                                  {teacher.specialistIds && teacher.specialistIds.length > 0 && (
                                    <div style={{
                                      display: 'flex',
                                      flexWrap: 'wrap',
                                      gap: '4px',
                                      ...
                                      {(teacher.specialistIds as unknown as Array<{name: string}>).slice(0, 2).map((spec, idx) => (
                                        <span key={idx} style={{
                                          fontSize: '10px',
                                          padding: '1px 6px',
                                          borderRadius: '4px',
                                ...
                                {selectedTeacherId === teacher._id && (
                                  <i className="bi bi-check-circle-fill" style={{ color: '#10b981', fontSize: '16px' }} />
                                )}
                              </div>
                            ))
                          )}
              ...
              <div className="calendar-controls" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                <div className="calendar-navigation">
                  ...
                  <h2>{format(currentDate, view === 'day' ? 'MMMM d, yyyy' : 'MMMM yyyy')}</h2>
                  <button
                    onClick={handleNext}
                    style={{
                      background: darkMode ? '#334155' : '#f1f5f9',
                ...
                {canCreateSchedule && (
                  <div style={{ display: 'flex', gap: '12px' }}>
                    ...
                    >
                      <i className="bi bi-plus-circle"></i> {isAdmin ? 'Create Schedule' : 'Request Schedule'}
                    </button>
                    {isAdmin && pendingSchedules.length > 0 && (
                      ...
                      >
                        <i className="bi bi-hourglass-split"></i> Review Pending ({pendingSchedules.length})
                      </button>
                    )}
                  </div>
                )}
          ...
          <aside className="events-section quick-actions-sidebar">
                ...
                {courses.slice(0, 8).map((course, index) => (
                  <div
                    ...
                    onMouseEnter={(e) => {
                      if (selectedCourseId !== course._id) {
                        e.currentTarget.style.background = darkMode ? '#374151' : '#f8fafc';
                        e.currentTarget.style.border = `1px solid ${darkMode ? '#4b5563' : '#e2e8f0'}`;
                      }
                    }}
                    onMouseLeave={(e) => {
                      if (selectedCourseId !== course._id) {
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ 
                        fontWeight: 500, 
                        fontSize: '12px',
                        color: selectedCourseId === course._id 
                    ...
                    {selectedCourseId === course._id && (
                      <i className="bi bi-check-circle-fill" style={{ color: '#3b82f6', fontSize: '14px' }} />
                    )}
                  </div>
                ))}
                {courses.length === 0 && (
                  <div style={{ 
                    ...
                    <i className="bi bi-folder2-open" style={{ fontSize: '20px', marginBottom: '8px', display: 'block' }} />
                ...
                <div style={{ maxHeight: '220px', overflowY: 'auto' }}>
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ 
                          fontWeight: 500, 
                          fontSize: '12px',
                          color: selectedTeacherId === teacher._id 
                          ...
                          {teacher.fullname || teacher.username}
                          {teacher.isVerified && (
                            <i className="bi bi-patch-check-fill" style={{ color: '#3b82f6', fontSize: '10px' }} />
                          )}
                        </div>
                        <div style={{ 
                          fontSize: '10px', 
                  ...
                  {teachers.length === 0 && (
                    <div style={{ 
                      ...
                      <i className="bi bi-person-x" style={{ fontSize: '20px', marginBottom: '8px', display: 'block' }} />
            ...
            <div className="sidebar-section">
              ...
              <div style={{ fontSize: '12px', color: darkMode ? '#94a3b8' : '#64748b' }}>
                ...
                {timeSlots.length === 0 && (
                  <div style={{ fontStyle: 'italic' }}>No time slots available</div>
                )}
              </div>
            </div>
                  ...
                  return (
                    <div key={schedule._id} className="event-item">
                      ...
                      <div className="event-details">
                        ...
                        <div className="event-location">{schedule.location || 'TBA'}</div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="sidebar-section">
              ...
              <div className="tips-list">
                <div className="tip-item"><i className="bi bi-star-fill"></i>Use Week view for a clear overview of your weekly schedule.</div>
                <div className="tip-item"><i className="bi bi-star-fill"></i>Click on Month view to see the entire month at a glance.</div>
                <div className="tip-item"><i className="bi bi-star-fill"></i>Use the navigation buttons to browse different dates.</div>
              </div>
            </div>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          border: `1px solid ${darkMode ? '#4b5563' : '#fcd34d'}`,
                          cursor: 'pointer',
                        }}
                        onClick={() => {
                          setSelectedSchedule(schedule);
                      ...
                      >
                        ...
                        <div style={{ fontSize: '11px', color: darkMode ? '#9ca3af' : '#92400e' }}>
                          {teacherName} • {schedule.dayOfWeek}
                        </div>
                        <div style={{ fontSize: '10px', color: darkMode ? '#6b7280' : '#b45309', marginTop: '4px' }}>
                          Requested: {safeFormatDate(schedule.requestedAt, 'MMM d, yyyy')}
                        </div>
                      </div>
                    );
                  })}
                  {pendingSchedules.length > 5 && (
                    <div style={{ fontSize: '12px', color: darkMode ? '#9ca3af' : '#6b7280', textAlign: 'center' }}>
      ...
      {showCreateModal && (
        <div className={`modal-overlay ${darkMode ? 'dark' : ''}`} onClick={() => setShowCreateModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-form">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (selectedCourse?.semesterId && typeof selectedCourse.semesterId === 'object') {
                      const semester = selectedCourse.semesterId;
                      if (semester.startDate) {
                        effectiveFrom = format(parseISO(semester.startDate), 'yyyy-MM-dd');
                      }
                      if (semester.endDate) {
                        effectiveTo = format(parseISO(semester.endDate), 'yyyy-MM-dd');
                      }
                    } else if (selectedCourse?.startDate) {
                      // Fallback to course dates if semester not populated
                      effectiveFrom = format(parseISO(selectedCourse.startDate), 'yyyy-MM-dd');
                      if (selectedCourse.endDate) {
                        effectiveTo = format(parseISO(selectedCourse.endDate), 'yyyy-MM-dd');
                      }
                    }
                ...
                >
                  ...
                  {courses.map(course => (
                    <option key={course._id} value={course._id}>
                      {course.title}
                      {course.semesterId && typeof course.semesterId === 'object' 
              ...
              <div className="form-group">
                ...
                <div style={{ 
                  ...
                  {(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'] as DayOfWeek[]).map(day => {
                    const existingSlot = createForm.slots.find(s => s.dayOfWeek === day);
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                ...
                                setCreateForm({
                                  ...createForm,
                                  slots: [...createForm.slots, { dayOfWeek: day, timeSlotId: '' }]
                                });
                              } else {
                                // Remove day
                                setCreateForm({
                                  ...createForm,
                                  slots: createForm.slots.filter(s => s.dayOfWeek !== day)
                                });
                              }
                            }}
                            style={{ width: '18px', height: '18px' }}
                          />
                          <span style={{ textTransform: 'capitalize', fontWeight: isSelected ? 500 : 400 }}>
                            {day}
                          </span>
                        </label>
                ...
                {createForm.slots.length > 0 && (
                  <div style={{ marginTop: '8px', fontSize: '13px', color: '#6366f1' }}>
              ...
              {(() => {
                ...
                const semester = selectedCourse?.semesterId && typeof selectedCourse.semesterId === 'object' 
                    ...
                    {semester && (
                      <div className="form-info" style={{ 
                        ...
                        <span style={{ marginLeft: '8px', opacity: 0.8 }}>
                          ({format(parseISO(semester.startDate), 'MMM dd, yyyy')} - {format(parseISO(semester.endDate), 'MMM dd, yyyy')})
                        </span>
                      </div>
                    )}
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setCreateForm({ ...createForm, requestNote: e.target.value })}
                />
              </div>
      ...
      {showApprovalModal && selectedSchedule && (
        <div className={`modal-overlay ${darkMode ? 'dark' : ''}`} onClick={() => { setShowApprovalModal(false); setSelectedSchedule(null); }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Review Schedule Request</h2>
              <button className="modal-close" onClick={() => { setShowApprovalModal(false); setSelectedSchedule(null); }}>×</button>
            </div>
            <div className="modal-body">
              ...
              <div className="detail-item">
                ...
                <span className="detail-value">{getCourseTitle(selectedSchedule.courseId)}</span>
              </div>

              <div className="detail-item">
                <span className="detail-label">Teacher:</span>
                <span className="detail-value">{getTeacherName(selectedSchedule.teacherId)}</span>
              </div>

              <div className="detail-item">
                <span className="detail-label">Day:</span>
                <span className="detail-value" style={{ textTransform: 'capitalize' }}>{selectedSchedule.dayOfWeek}</span>
              </div>
              ...
              <div className="detail-item">
                ...
                <span className="detail-value">{selectedSchedule.location || 'Not specified'}</span>
              </div>

              <div className="detail-item">
                <span className="detail-label">Effective From:</span>
                <span className="detail-value">{safeFormatDate(selectedSchedule.effectiveFrom, 'MMM d, yyyy')}</span>
              </div>

              {selectedSchedule.effectiveTo && (
                <div className="detail-item">
                  <span className="detail-label">Effective To:</span>
                  <span className="detail-value">{safeFormatDate(selectedSchedule.effectiveTo, 'MMM d, yyyy')}</span>
                </div>
              )}
            ...
            {selectedSchedule.status === 'pending' && (
              <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '16px 24px', borderTop: `1px solid ${darkMode ? '#4b5563' : '#e5e7eb'}` }}>
                <button
                  className="btn-delete"
                  onClick={() => handleApproveSchedule(selectedSchedule._id, false, 'Rejected by admin')}
                  disabled={actionLoading}
                >
                ...
                <button
                  className="btn-primary"
                  onClick={() => handleApproveSchedule(selectedSchedule._id, true, 'Approved by admin')}
                  disabled={actionLoading}
      ...
      {showEditModal && selectedSchedule && (
        <div className={`modal-overlay ${darkMode ? 'dark' : ''}`} onClick={() => { setShowEditModal(false); setSelectedSchedule(null); }}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Schedule Details</h2>
              <button className="modal-close" onClick={() => { setShowEditModal(false); setSelectedSchedule(null); }}>×</button>
            </div>
            <div className="modal-body">
              <div className="detail-item">
                <span className="detail-label">Course:</span>
              ...
              {selectedSchedule.approvedBy && (
                <div className="detail-item">
                  ...
                  <span className="detail-value">{safeFormatDate(selectedSchedule.approvedAt, 'MMM d, yyyy HH:mm')}</span>
                </div>
              )}
            </div>
