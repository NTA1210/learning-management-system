...
const StaticCourseTab: React.FC<StaticCourseTabProps> = ({
  courseId,
  darkMode,
}) => {
  ...
  const droppedRate = Number(
    statistics.droppedRate ?? Math.max(0, 100 - passRate)
  );
    ...
    <div className="space-y-8">
      <div>
        ...
        {semester && (
          <div className="mt-2 inline-flex items-center gap-2">
            ...
            >
              {semester.type?.toUpperCase()} {semester.year}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          ...
          style={{
            backgroundColor: darkMode ? "rgba(31,41,55,0.8)" : "#f9fafb",
            border: darkMode
      ...
      {/* Students list */}
      <div>
        <StudentsEnrollList courseId={courseId} darkMode={darkMode} />
      </div>
    </div>
  );
};
